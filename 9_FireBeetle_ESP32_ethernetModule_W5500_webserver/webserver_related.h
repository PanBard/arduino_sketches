...
void handle_x_servo_movement()
{
  deserialize_post_message();
  move_x_servo(postMessage);
}

void handle_y_servo_movement()
{
  deserialize_post_message();
  move_y_servo(postMessage);
}
void wifi_start() {
  Serial.println();
  ...
  server.on("/x_servo", HTTP_POST, handle_x_servo_movement); 
  server.on("/y_servo", HTTP_POST, handle_y_servo_movement); 
